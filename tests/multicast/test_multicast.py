from async_rx.multicast import multicast
...
def test_multicast(kernel):
    ...
    a_multicast = multicast(an_observable=rx_range(start=0, stop=100), subject_handler=subject_handler, connection_handler=connection_handler)
    assert a_multicast
...
def test_multicast_with_ref_count(kernel):
    ...
    a_multicast = multicast(an_observable=rx_range(start=0, stop=100), subject_handler=subject_handler, connection_handler=connection_handler).ref_count()
    assert a_multicast
    assert not connection_handler.connected
...
def test_multicast_with_ref_count_on_subject(kernel):
    ...
    a_multicast = multicast(an_observable=a_subject, subject_handler=subject_handler, connection_handler=connection_handler).ref_count()
    assert a_multicast
    assert not connection_handler.connected
