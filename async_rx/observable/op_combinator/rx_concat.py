...
def rx_concat(*observables: Observable) -> Observable:
    ...
    async def _subscribe(an_observer: Observer) -> Subscription:
        ...
        async def _on_completed() -> None:
            ...
            if _source:
                obs = _source.pop(0)
                _current_subscription = obs.subscribe(_observer)
            else:
        ...
        _observer = observer(on_next=an_observer.on_next, on_completed=_on_completed, on_error=_on_error)

        # initiate
        _current_subscription = _source.pop(0).subscribe(_observer)
