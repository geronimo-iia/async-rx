...
def multicast(
    ...
    subject_factory: SubjectFactory = subject,
) -> ConnectableObservable:
    """Create a Connectable Observable.

    A multicasted Observable uses a Subject under the hood to make multiple
    ...
    _subject = subject_factory(subject_handler=SubjectHandlerDefinition(on_subscribe=_on_subscribe, on_unsubscribe=_on_unsubscribe))
    ...
    _connectable_observable = ConnectableObservableDefinition(connect=_connect, ref_count=_ref_count_handler, subscribe=_subject.subscribe)
